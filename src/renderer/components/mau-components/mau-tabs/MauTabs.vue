<template>
  <div class="mau-tabs">
    <div class="hidden-lg-down">
      <nav class="nav nav-pills row">
        <div class="nav-item col" v-on:click="setActive(name)"
             :class="{active: name === currentActive, 'has-error': isInErrorNames(name)}" v-for="name in names">
          <span class="nav-link"><h5>{{name}}</h5></span>
        </div>
      </nav>
      <div class="track"><div :class="underscoreClass"></div></div>
    </div>
    <div class="tab h-100">
      <div class="tab-pane h-100"
           :class="{active: name === currentActive}" v-for="name in names">
        <slot :name="name" class="tab-content"></slot>
      </div>
    </div>
  </div>
</template>

<script>
  export default {
    name: 'MauTabs',
    inject: {
      $validator: '$validator'
    },
    data () {
      return {
        currentActive: this.names[0]
      }
    },
    props: {
      names: {
        type: Array,
        required: true
      },
      onSave: {
        type: Function
      },
      errorNames: {
        type: Array,
        default: function () {
          return []
        }
      }
    },
    computed: {
      underscoreClass () {
        return 'underscore-' + this.names.length + '-' + this.names.indexOf(this.currentActive)
      }
    },
    methods: {
      setActive (name) {
        this.currentActive = name
      },
      saveInfo () {
        if (this.onSave) {
          this.onSave()
        }
      },
      isInErrorNames (name) {
        return (this.errorNames.indexOf(name) > -1)
      }
    }
  }
</script>

<style lang="scss">
  @import "../../../sass/variables";

  .mau-tabs {
    background-color: white;
    height: 100%;
    .simple-select {
      padding-top: 1.5rem;
    }
    .nav {
      margin: 0;
      .nav-item {
        flex-grow: 1;
        text-align: center;
        padding: 0;
        .nav-link {
          padding: 0;
          color: $gray;
          transition: all .3s ease;
        }
        &.has-error {
          .nav-link {
            color: $mf-error-color;
          }
        }
        &:hover {
          cursor: pointer;
          .nav-link {
            color: $vue-darkest-blue;
          }
        }
        &.active {
          .nav-link {
            color: $vue-darkest-blue;
          }
        }
      }
    }
    .track {
      overflow: hidden;
      width: 100%;
      height: .125rem;
      position: relative;
      div[class^='underscore-']{
        background-color: $brand-primary;
        height: .125rem;
        position: absolute;
      }
      $koeff: 0.8;
      @for $all from 1 through 10 {
        $width: 1/$all;
        div[class^='underscore-#{$all}'] {
          width: $width * $koeff * 100%;
          transition: left .5s cubic-bezier(0.68, -0.55, 0.265, 1.55);
        }
        @for $place from 0 through $all {
          .underscore-#{$all}-#{$place} {
            left: ($place + (1 - $koeff)/2) * $width * 100%;
          }
        }
      }
    }
    .tab {
      padding-bottom: $tab-content-pb;
      padding-top: $tab-content-pt;
      > .tab-pane {
        width: 100%;
        display: none;

        &.active {
          display: block;
        }
      }
      .tab-content {
        height: 100%;
      }
    }
    .button-container {
      padding-bottom: $tab-content-pb;
    }
  }
</style>
