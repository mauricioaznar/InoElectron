  ...
  export default {
        ...
        alwaysActions: [
          {
            name: 'Create',
            icon: 'fa fa-plus',
            path: RouteObjectHelper.createPath(EntityTypes.ROLL_ORDER_PRODUCTION, 'create')
          }
        ],
        buttons: []
      }
    ...
    methods: {
          ...
          this.actions = this.alwaysActions.concat([
            {
              name: 'Edit',
              icon: 'fa fa-edit',
              path: RouteObjectHelper.createPath(EntityTypes.ROLL_ORDER_PRODUCTION, 'edit') + '/' + id
            },
            {
              name: 'View',
              icon: 'fa fa-eye',
              path: RouteObjectHelper.createPath(EntityTypes.ROLL_ORDER_PRODUCTION, 'view') + '/' + id
            }
          ])
          this.buttons = [{
            name: 'Delete',
      ...
      confirmDelete: function () {
        let id = this.$route.params[GlobalEntityIdentifier]
        GenericApiOperations.del(EntityTypes.ROLL_ORDER_PRODUCTION.apiName, id).then(result => {
