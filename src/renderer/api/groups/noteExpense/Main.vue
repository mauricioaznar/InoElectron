  ...
  export default {
    ...
    methods: {
          ...
          this.actions = this.alwaysActions.concat([
            {
              name: 'Edit',
              icon: 'fa fa-edit',
              path: RouteObjectHelper.createPath(EntityTypes.NOTE_EXPENSE, 'edit') + '/' + id
            },
            {
              name: 'View',
              icon: 'fa fa-eye',
              path: RouteObjectHelper.createPath(EntityTypes.NOTE_EXPENSE, 'view') + '/' + id
            }
          ])
          this.buttons = [{
            name: 'Delete',
      ...
      confirmDelete: function () {
        let id = this.$route.params[GlobalEntityIdentifier]
        GenericApiOperations.del(EntityTypes.NOTE_EXPENSE.apiName, id).then(result => {
