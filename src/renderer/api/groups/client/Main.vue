    ...
    methods: {
      actionClicked: function (action) {
        if (action.name === 'Create') {
          this.$router.push({name: RouteObjectHelper.createName(EntityTypes.CLIENT, 'create')})
        }
        if (action.name === 'Edit') {
          let id = this.$route.params[GlobalEntityIdentifier]
          this.$router.push({
            name: RouteObjectHelper.createName(EntityTypes.CLIENT, 'edit'),
            params: {
        ...
        if (action.name === 'View') {
          let id = this.$route.params[GlobalEntityIdentifier]
          this.$router.push({
            name: RouteObjectHelper.createName(EntityTypes.CLIENT, 'view'),
            params: {
              [GlobalEntityIdentifier]: id
            }})
        }
      ...
      confirmDelete: function () {
        let id = this.$route.params[GlobalEntityIdentifier]
        ApiOperations.del(EntityTypes.CLIENT, id).then(result => {
