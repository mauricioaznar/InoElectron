  ...
  export default {
    ...
    methods: {
          ...
          this.actions = this.alwaysActions.concat([
            {
              name: 'Edit',
              icon: 'fa fa-edit',
              path: RouteObjectHelper.createPath(EntityTypes.CLIENT, 'edit') + '/' + id
            },
            {
              name: 'View',
              icon: 'fa fa-eye',
              path: RouteObjectHelper.createPath(EntityTypes.CLIENT, 'view') + '/' + id
            }
          ])
          this.buttons = [{
            name: 'Delete',
      ...
      confirmDelete: function () {
        let id = this.$route.params[GlobalEntityIdentifier]
        GenericApiOperations.del(EntityTypes.CLIENT.apiName, id).then(result => {
