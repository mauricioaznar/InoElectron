      ...
      methods: {
        setInitialBootstrapData: function () {
          ...
          Promise.all([
            ...
            this.setMachinesData(productionData, machines)
            this.setMaterialsData(productionData, materials)
            let vm = this
        ...
        setMachinesData: function (machinesData, machines) {
          ...
          this.machineItems.forEach(yearMachineItem => {
            yearMachineItem['monthItems'].forEach(monthMachineItem => {
              monthMachineItem = Object.assign(monthMachineItem, getTableRowDefaultMachineData(machines), {total: 0})
              monthMachineItem['dayItems'].forEach(dayMachineItem => {
                dayMachineItem = Object.assign(dayMachineItem, getTableRowDefaultMachineData(machines), {total: 0})
              })
            })
          })
        ...
        setMaterialsData: function (materialsData, materials) {
          ...
          this.materialItems.forEach(yearMaterialItem => {
            yearMaterialItem['monthItems'].forEach(monthMaterialItem => {
              monthMaterialItem = Object.assign(monthMaterialItem, getTableRowDefaultMaterialData(materials), {total: 0})
              monthMaterialItem['dayItems'].forEach(dayMaterialItem => {
                dayMaterialItem = Object.assign(dayMaterialItem, getTableRowDefaultMaterialData(materials), {total: 0})
              })
            })
          })
